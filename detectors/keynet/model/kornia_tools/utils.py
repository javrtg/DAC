...
# modifications for adapting to kornia 0.6.9
...
def laf_from_center_scale_ori(xy: torch.Tensor, scale: torch.Tensor, ori: torch.Tensor) -> torch.Tensor:
    ...
    for var_name, var, req_shape in zip(names,
        ...
        # type: ignore  # because it does not like len(tensor.shape)
        if len(var.shape) != len(req_shape):
            raise TypeError(
                "{} shape should be must be [{}]. "
                "Got {}".format(var_name, str(req_shape), var.size()))
        # type: ignore # because it wants typing for dim
        for i, dim in enumerate(req_shape):
            ...
            if var.size(i) != dim:
                raise TypeError(
                    "{} shape should be must be [{}]. "
                    "Got {}".format(var_name, str(req_shape), var.size()))
    unscaled_laf: torch.Tensor = torch.cat([kornia.geometry.conversions.angle_to_rotation_matrix(ori.squeeze(-1)),
    ...
    while min(cur_img.size(2), cur_img.size(3)) >= PS:
        ...
        for i in range(B):
            ...
            grid = generate_patch_grid_from_normalized_LAF(
                ...
                PS)
            # Keynet pytorch version is 1.2.0 -> align_corners was = True by
            # default. This was reversed in posterior versions of pytorch.
            # Thus, here we add align_corners = True.
